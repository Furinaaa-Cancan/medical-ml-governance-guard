...
<br>
<strong>23 Model Families</strong> &middot; <strong>16 Real Medical Datasets (630K+ rows)</strong> &middot; <strong>335 NC+CM Peer Review PDFs · 105 Curated with Concerns</strong> &middot; <strong>21 Analysis Tools</strong>
<br><br>
...
│  peer-review-kb.json (105 curated +230 pend)│  Consumer: SQL / JSON
...
> **MLGG is not yet another ML toolkit.** It is an AI co-review system meeting top-journal review standards &mdash; 33 fail-closed gates + 105 NC+CM curated reviews (Nature Communications + Communications Medicine; 449 structured concerns; 230 additional PDFs cataloged and pending extraction) as a knowledge base. Every recommendation can cite reviewer quotes as evidence.
...
| **Layer 3: Clinical Semantic Review + Peer Review Evidence** | AI agent understands code semantics + 105-paper curated peer-review KB (449 concerns) + **issue-code-aware retrieval** | Post-discharge variables predicting post-discharge outcomes, HbA1c definition leakage, missing subgroup calibration. RAG re-ranks by keyword overlap with the actual failure codes — not just severity |

**Peer Review Knowledge Base:**

Structurally extracted 449 review opinions from 105 NC + CM medical ML papers (230 additional PDFs cataloged but pending extraction). **Retrieval precision refactored 2026-04**: the previous `retrieve_by_gate(gate_name)` filtered by `mlgg_gates` then sorted by severity alone (~20% precision on clinical_metrics_gate's ppv-specific failures). The replacement `retrieve_for_failure(gate_name, issue_codes)` tokenizes the failure code list, filters stopwords, and re-ranks by `3 × tag_overlap + text_overlap`; falls back to severity-only when no keywords match so coverage never regresses to empty.
...
> When MLGG finds an issue in your code, it doesn't just say "violated rule E02" &mdash; it tells you: *"NC+CM reviewers requested improved evaluation metrics 128 times (28.5%) across 105 papers. This is the most frequently raised concern category."*
...
| **Peer Review Evidence Base** | 105 NC+CM papers &times; 449 structured review opinions, retrieved by gate/tag/severity (230 additional PDFs cataloged but pending) | Each recommendation cites original text |
...
│   │   ├── _peer_review_retrieval.py     #   449 review opinions BM25 retrieval + tag synonym expansion
...
│   │   ├── peer_review_lookup.py         #   105 NC+CM papers × 449 review opinions
...
- Cite 105 NC+CM peer-review papers (449 structured review opinions) as evidence
