...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6649%20passed-brightgreen" alt="Tests">
...
| HbA1c both defines diabetes and serves as predictor | Perfect leakage, model learns the definition itself | Gate C02: Definition column forced exclusion (scope is **spec-driven** — place glucose under the diabetes target's `defining_variables` block to flag it for diabetes only; under `global_forbidden_variables` it is flagged for every target. The gate does not auto-scope by disease) |
...
| **Layer 2: 33 Fail-Closed Gates** | Runtime validation, JSON report output | Patient cross-split, calibration ECE > 0.1, EPV < 10, CI width > 0.20; **post-index feature-name detection** (time_in_hospital / num_medications / discharge / ventilation / vasopressor); **spec-scoped definition-variable matching** (glucose flagged for diabetes only when placed under that target's block — the gate does not auto-scope by disease) |
...
> **Asymmetric LLM advisory layer (optional, opt-in):** `mlgg llm-review [--rag] [--live]` hands the gate evidence above (plus retrieved peer-review precedent when `--rag` is set) to an LLM that synthesizes methodology concerns into `evidence/llm_review_report.json`, which publication_gate folds into the verdict by convention. **This layer can only raise concerns — it can never turn a gate failure into a pass** (`final = min(gate, llm)`): a blocking concern caps the certification tier to `none`, an advisory one is only recorded. It defaults to a deterministic, no-network double (reproducible); only `--live` makes a real Claude call (needs `ANTHROPIC_API_KEY`). It is not on the default `mlgg workflow` path — invoke it explicitly.
...
**Offline evaluation self-exclusion (NCPR / PDF runner):** paper-audit and NCPR benchmarks must use leave-one-paper-out whenever they evaluate papers already represented in the KB: the current paper's `paper_id` / `id` / `paper_doi` / `doi` are passed to RAG through `excluded_paper_ids`, and the dense index builder, BM25 retrieval path, and post-union hybrid candidate pool all filter those identifiers. The PDF-backed runner also accepts extra `paper_identifiers` (for DOI / aliases) and threads them into every Methods chunk query, so a paper cannot retrieve its own reviewer concerns as the answer key. Empty or unknown identifiers are ignored and do not affect ordinary offline queries.
