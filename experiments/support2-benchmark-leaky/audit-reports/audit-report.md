...
|---|-----------|-------|-----|-------|
| 1 | Data Integrity | 1.44 | 12 | ✗ |
| 2 | Leakage Prevention | 0.0 | 15 | ✗ |
| 3 | Pipeline Isolation | 0.0 | 12 | ✗ |
...
**Coverage**: 0/17 required items (0%)  

| Item ID | Label | AI-Specific | Status |
|---------|-------|-------------|--------|
| 1 | Title |  | ? Not Assessed |
| 4 | Source of Data |  | ? Not Assessed |
...
|--------|-----------|----------------|
| Participant Selection | ? unclear | `split_protocol_gate, external_validation_gate, sample_size_gate` |
...
| Analysis | ? unclear | `leakage_gate, tuning_leakage_gate, split_protocol_gate` |
...
#### [ERROR] Selected feature set includes lineage-forbidden features.

**Source**: Gate: `feature_engineering_audit`  
**Error Code**: `feature_selection_data_leakage`  
**Root Cause**: Gate feature_engineering_audit failed: Selected feature set includes lineage-forbidden features.  

**Fix**: Review gate requirements for feature_engineering_audit and address the listed failure.  
**Prevention**: Run gates regularly during development, not just at publication.  
...
### [P2] Improve Pipeline Isolation dimension score (0.0/12)

**Severity**: INFO  

**Fix**: Address all failing checks in the Pipeline Isolation dimension.
...
- ✗ Patient-level data splitting (no patient in multiple splits)
