...
register_remediations({
    "missing_robustness_report": "Provide --robustness-report pointing to a valid robustness_report.json generated by the evaluation pipeline.",
    "invalid_robustness_report": "Fix JSON syntax in robustness_report.json. Use a JSON linter.",
    "robustness_missing_bucket_rows": "Ensure robustness_report contains non-empty 'slices' (time_slices) and 'groups' (patient_hash_groups) arrays.",
    "robustness_invalid_bucket_row": "Each row must have positive integer 'n', non-negative 'positive_count', and a 'metrics' object with 'pr_auc'.",
    "robustness_non_finite_metric": "Replace NaN/Inf metric values with valid finite numbers. Check evaluation code for division-by-zero.",
    "robustness_metric_out_of_range": "PR-AUC must be in [0,1]. Check for metric computation errors.",
    "robustness_pr_auc_drop_exceeds_threshold": "Model performance drops excessively in some time slices or patient groups. Consider time-aware training or group-balanced sampling.",
    "robustness_pr_auc_range_exceeds_threshold": "High variance across slices/groups. Consider regularization or domain adaptation techniques.",
    "robustness_missing_overall_metric": "overall_test_metrics.pr_auc must be a finite number in robustness_report.",
    "robustness_missing_bucket": "robustness_report must include both 'time_slices' and 'patient_hash_groups' objects.",
    "robustness_summary_mismatch": "Declared summary values don't match computed values. Re-generate robustness_report.",
