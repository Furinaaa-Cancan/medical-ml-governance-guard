...
register_remediations({
    ...
    "missing_policy_spec": "Imbalance policy spec file not found; provide a valid path to the JSON file.",
    "invalid_policy_spec": "Imbalance policy spec JSON is malformed; fix syntax and ensure root is an object.",
    "invalid_policy_field": "A required policy field has an invalid type or value; check the field against the expected schema.",
    "unsupported_imbalance_strategy": "Use a supported imbalance strategy: none, class_weight, random_oversample, random_undersample, smote, adasyn, or auto.",
    "invalid_fit_scope": "Set fit_scope to train_only, cv_inner_train_only, or fold_train_only to prevent leakage.",
    "test_usage_not_forbidden": "Set forbid_test_usage to true in the policy spec to prevent test-set contamination.",
    "invalid_postprocessing_split": "Use an approved non-test scope (valid, cv_inner, nested_cv) for threshold/calibration selection.",
    "test_split_used_for_postprocessing": "Never use the test split for threshold or calibration selection; switch to valid or cv_inner.",
    "train_split_used_for_postprocessing": "Do not reuse the train split for threshold/calibration; use a held-out valid or cv_inner split.",
    "valid_split_required_but_missing": "Policy requires a valid split for post-processing but none was provided; supply a --valid path.",
    "missing_resampling_scope": "Resampling strategy requires a resampling_applied_to list specifying which splits are resampled.",
    "resampling_scope_leakage": "Resampling must be applied only to the train split; remove other splits from resampling_applied_to.",
    "unexpected_resampling_scope": "Non-resampling strategy must not declare active resampling scopes; remove resampling_applied_to.",
    "split_label_read_error": "Failed to read split CSV labels; verify the file path, encoding, and target column name.",
    "empty_split": "Split CSV must not be empty; ensure the file contains header and data rows.",
    "invalid_labels": "Split contains non-binary or unparseable label values; ensure target column has only 0/1.",
    "single_class_split": "Split must contain both positive and negative classes for valid model evaluation.",
    "insufficient_minority_samples": "Train minority class count is below the policy minimum; collect more minority-class samples.",
    "imbalance_unmitigated": "Severe class imbalance detected with strategy 'none'; apply class_weight or resampling.",
    "missing_evaluation_report": "Evaluation report file not found; provide a valid path for imbalance reconciliation.",
    "invalid_evaluation_report": "Evaluation report JSON is malformed or not an object; fix the file and retry.",
    "evaluation_report_imbalance_metadata_missing": "Add metadata.imbalance.selected_strategy to the evaluation report for reconciliation.",
    "evaluation_report_imbalance_strategy_invalid": "Evaluation report selected_strategy is not a recognized imbalance strategy; use a supported value.",
    "imbalance_execution_policy_mismatch": "Executed imbalance strategy does not match the policy; align the pipeline with the declared strategy.",
})
