...
register_remediations({
    ...
    "invalid_policy_field": "A required policy field has an invalid type or value; check the field against the expected schema.",
    "invalid_policy_range": "Policy ratio or numeric field is outside the allowed range; adjust to within bounds.",
    "missing_policy_spec": "Missingness policy spec file not found; provide a valid path to the JSON file.",
    "invalid_policy_spec": "Missingness policy spec JSON is malformed; fix syntax and ensure root is an object.",
    "invalid_evaluation_report": "Evaluation report JSON is malformed or unparseable; fix the file and retry.",
    "unsupported_missingness_strategy": "Use a supported missingness strategy: none, simple, simple_with_indicator, mice, mice_with_scale_guard, missforest, knn, or drop_rows.",
    "indicator_flag_required": "Set add_missing_indicators=true when using simple_with_indicator strategy.",
    "invalid_imputer_fit_scope": "Set imputer_fit_scope to train_only, cv_inner_train_only, or fold_train_only to prevent leakage.",
    "test_usage_not_forbidden": "Set forbid_test_usage to true in the policy spec to prevent test-set contamination.",
    "test_used_for_imputer_fit": "Imputer must not be fit on test data; set test_used_for_fit to false.",
    "valid_used_for_imputer_fit": "Imputer fit uses valid split; strict protocols prefer train-only fitting.",
    "target_used_in_imputation": "Remove the target/outcome column from the imputation model to prevent leakage.",
    "split_missingness_read_error": "Failed to profile split missingness; verify the CSV path, encoding, and header.",
    "empty_split": "Split CSV must not be empty; ensure the file contains header and data rows.",
    "target_missing_values": "Target column contains missing values; remove or impute missing target rows before training.",
    "no_features_after_exclusions": "No usable feature columns remain after excluding ignored columns; review --ignore-cols.",
    "strategy_not_scalable_for_large_data": "Chosen imputation strategy is not approved for this data scale; use a simpler method like simple_with_indicator.",
    "mice_row_scale_exceeded": "MICE is not approved beyond the configured row limit; reduce data or switch to a simpler strategy.",
    "mice_feature_scale_exceeded": "MICE is not approved beyond the configured feature limit; reduce features or switch strategy.",
    "mice_scale_guard_violation": "MICE scale guard evidence is missing or inconsistent; provide correct scale_guard_evidence in the policy.",
    "mice_scale_guard_unexpected_fallback": "Scale guard fallback triggered although size thresholds were not exceeded; verify scale_guard_evidence.",
    "missing_imputation_execution_evidence": "Add metadata.imputation block to evaluation_report for publication-grade missingness execution audit.",
    "imputation_execution_mismatch": "Executed imputation strategy does not match the policy expectation; align the pipeline with the declared strategy.",
    "missingness_unhandled": "Missing values exist but strategy is 'none' without complete-case analysis; enable imputation or set complete_case_analysis=true.",
    "feature_missingness_too_high": "Feature missingness exceeds the policy threshold; impute, drop the feature, or add it to allowed_high_missing_features.",
    "insufficient_non_missing_samples": "Feature has too few non-missing samples; collect more data or drop the feature.",
    "missing_indicator_recommended": "High-missingness feature lacks a missing-indicator flag; set add_missing_indicators=true.",
    "mnar_tipping_point_invalid": "mnar_sensitivity.tipping_point should be a finite number representing the smallest delta that flips the conclusion.",
    "missingness_shift": "Missingness ratio shift between splits exceeds tolerance; investigate distribution drift or stratification issues.",
})
