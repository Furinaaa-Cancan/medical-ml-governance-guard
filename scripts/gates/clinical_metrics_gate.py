...
register_remediations({
    ...
    "invalid_tolerance": "Provide a finite non-negative tolerance value.",
    "invalid_evaluation_report": "Supply a valid, parseable evaluation report JSON file.",
    "invalid_performance_policy": "Supply a valid, parseable performance policy JSON file with allowed threshold settings.",
    "performance_policy_missing_required_metric": "Add the full mandatory clinical metric panel to performance_policy.required_metrics.",
    "missing_split_metrics": "Include split_metrics with train/valid/test blocks in the evaluation report.",
    "missing_threshold_selection": "Add a threshold_selection block with selection_split to the evaluation report.",
    "missing_threshold_selection_split": "Specify threshold_selection.selection_split (valid/cv_inner/nested_cv).",
    "test_split_used_for_threshold_selection": "Use valid, cv_inner, or nested_cv for threshold selection — never train or test.",
    "threshold_selection_policy_mismatch": "Align evaluation report threshold_selection.selection_split with the performance policy.",
    "invalid_threshold_selection_value": "Set threshold_selection.selected_threshold to a numeric value in [0, 1].",
    "threshold_guard_constraints_not_met": "Ensure the guard split satisfies all clinical floor constraints when using cv_inner selection.",
    "missing_split_metric_block": "Populate split_metrics.<split>.metrics as an object with all required metric keys.",
    "missing_confusion_matrix": "Populate split_metrics.<split>.confusion_matrix with tp/fp/tn/fn counts.",
    "invalid_confusion_matrix": "Ensure confusion_matrix values are non-negative integers summing to > 0.",
    "invalid_split_sample_count": "Set split_metrics.<split>.n_samples to a positive integer matching the confusion matrix total.",
    "confusion_matrix_row_count_mismatch": "Make confusion matrix total consistent with n_samples and data fingerprint row_count.",
    "missing_required_metric": "Add the missing required metric to the evaluation report split metrics.",
    "metric_out_of_range": "Verify metric computation — value must be within [0, 1] for unit-range metrics.",
    "metric_formula_uncomputable": "Check for zero denominators in the confusion matrix that prevent metric derivation.",
    "metric_formula_mismatch": "Recompute the metric from the confusion matrix — reported and derived values must agree.",
    "missing_top_level_metrics": "Ensure evaluation_report.metrics mirrors split_metrics.test.metrics for all required metrics.",
    "top_level_test_metric_mismatch": "Sync top-level metrics with test-split metrics so they match within tolerance.",
    "missing_threshold_selection_snapshot": "Include selected_metrics_on_valid and selected_confusion_on_valid in threshold_selection.",
    "threshold_selection_metric_mismatch": "Ensure threshold_selection snapshot metrics match split_metrics.valid.metrics.",
    "threshold_selection_confusion_mismatch": "Ensure threshold_selection snapshot confusion matrix matches split_metrics.valid.confusion_matrix.",
    "clinical_floor_sensitivity_not_met": "Sensitivity is below the clinical floor — retune the operating point or improve recall.",
    "clinical_floor_npv_not_met": "NPV is below the clinical floor — reduce false negatives to raise negative predictive value.",
    "clinical_floor_specificity_not_met": "Specificity is below the clinical floor — reduce false positives to raise specificity.",
    "clinical_floor_ppv_not_met": "PPV is below the clinical floor — reduce false positives to raise positive predictive value.",
})
