...
def ensure_project_root_marker(project_root: Path) -> None:
    """Drop a minimal pyproject.toml at project root if no marker exists.

    request_contract_gate looks for SKILL.md / .git / pyproject.toml /
    .mlgg_model_key ascending from the request file to anchor its path
    sandbox. Without one, the gate falls back to request_base.parent and
    emits a path_sandbox_unanchored warning, which fails workflow_bootstrap
    in --strict mode.

    Real users running mlgg in their own repo always have one (.git is
    near-universal), but mlgg init creating a fresh /tmp/foo project for
    onboarding/demo doesn't, so this function ensures every init'd project
    is sandbox-anchored. pyproject.toml is the lightest legitimate option:
    git-init would be heavier and committing to a real config inside
    SKILL.md would be misleading.
    """
    for marker in ("SKILL.md", ".git", "pyproject.toml", ".mlgg_model_key"):
        if (project_root / marker).exists():
            return
    pyproject = project_root / "pyproject.toml"
    pyproject.write_text(
        "# Auto-generated by mlgg init to anchor request_contract_gate's "
        "path sandbox.\n"
        "# Replace with your real pyproject.toml when adopting mlgg in a "
        "production project.\n"
        "[project]\n"
        'name = "mlgg-project"\n'
        'version = "0.0.0"\n',
...
def main() -> int:
    ...
    ensure_project_root_marker(project_root)
