...
**Phase 规则和评审循环已模块化**。Agent 交互时：
- 输入 `/mlgg` → 加载 `.claude/commands/mlgg.md`（精简状态机 + 评审循环）
- 每个 Phase 开始时 → 按需读取 `references/skill/phase-N.md`
- 评审协议 → `references/skill/review-protocol.md`
- 审计模式 → `references/skill/audit-mode.md`

以下内容为 **操作参考**（dispatch 表、契约、资源清单），不再重复 Phase 流程。

---

## Quick Dispatch（用户意图 → 操作命令）
...
Phase 1-9 的详细规则、Gate 命令、评审检查清单已模块化到 `references/skill/` 目录：

```
references/skill/
├── review-protocol.md    # 评审循环协议（核心）
├── phase-1.md            # 数据理解 & 队列定义
├── phase-2.md            # 数据划分
├── phase-3.md            # 预处理
├── phase-4.md            # 特征选择
├── phase-5.md            # 模型训练
├── phase-6.md            # 评估
├── phase-7.md            # SHAP 可解释性
├── phase-8.md            # 公平性
├── phase-9.md            # 报告
└── audit-mode.md         # 审计模式
```

Agent 在 `/mlgg` 流程中按需读取对应 Phase 文件，不需要一次性全部加载。

### 疾病定义知识库

`references/disease-definition-knowledge-base.json` 覆盖 10 种常见疾病：
T2D, 高血压, 冠心病, CKD, 心衰, 脑卒中, COPD, 抑郁症, 癌症, 心房颤动, 30天再入院

### Gate 失败恢复

```
1. 查看报告: python3 scripts/tools/explain_gate.py --report evidence/<gate>_report.json
2. 查错误库: references/error-knowledge-base.json
3. 修复 → 重跑: python3 scripts/orchestration/mlgg.py workflow --strict
```
